...
def test_standard():
    ...
    assert symbolic.standard([]) is None
    assert symbolic.standard(0) == '0'
...
def test_empty_expression():
    """Initialize an expression with no terms."""
    args = [None, [], (), '']
    for arg in args:
        this = symbolic.expression(arg)
        assert this.isempty
        assert not this
    assert not symbolic.expression(0).isempty
...
@pytest.mark.expression
def test_expression_ismultiplicative():
    """Test the expression object against the multiplicative protocol."""
    expression = symbolic.expression([])
    assert isinstance(expression, Multiplicative)
...
def test_init_collection():
    ...
    for string, terms in cases.items():
        from_terms = symbolic.expression(terms)
        assert symbolic.expression(string) == from_terms
        parts = [
            symbolic.OperandFactory().create(term)
            for term in terms
        ]
        from_parts = symbolic.expression(parts)
        assert symbolic.expression(string) == from_parts
...
def test_formatted_expression():
    ...
    testable = {
        '(4a0)^2 * (a1*a2) / (4a3 * a4^2 * (a5/a6))': {
            None: ['4', 'a0^2', 'a1', 'a2', 'a3^-1', 'a4^-2', 'a5^-1', 'a6'],
            ('tex', 'TeX', 'latex', 'LaTeX'): [
                '4',
                '$\\mathrm{a0}^{2}$',
                '$\\mathrm{a1}$',
    ...
    for string, current in testable.items():
        expresssion = symbolic.expression(string)
        for style, terms in current.items():
            if style is None:
